...
interface RaceTrackProps {
  ...
  opponentProgress: number; // Set to -1 to hide
  playerName?: string;
  opponentName?: string;
}
...
const RaceTrack: React.FC<RaceTrackProps> = ({ 
  ...
  return (
    <div className="w-full bg-slate-900/80 p-8 rounded-3xl border-2 border-slate-800 shadow-2xl overflow-hidden relative group/track">
      {/* Asphalt Texture and Road Lines */}
      <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'radial-gradient(#ffffff 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>
      
      {/* Starting Line Skid Marks */}
      ...
      <div className="flex flex-col gap-12 relative z-10 py-4">
        ...
        <div className={`relative h-20 flex items-center ${isPractice ? 'justify-center' : ''}`}>
          ...
          >
            <div className="flex flex-col items-center">
               ...
               <div className={`relative ${isMoving(playerProgress) ? 'engine-rumble' : ''}`}>
                  {isMoving(playerProgress) && <div className="absolute -left-6 top-1/2 -translate-y-1/2 w-12 h-4 bg-sky-500/40 blur-md rounded-full nitro origin-right"></div>}
                  {isMoving(playerProgress) && (
                    <>
                      <div className="smoke" style={{ left: '-10px', top: '20px', animationDelay: '0s' }}></div>
                      <div className="smoke" style={{ left: '-15px', top: '15px', animationDelay: '0.2s' }}></div>
                    </>
                  )}
        ...
        {/* Opponent Lane (Hidden in Practice) */}
        {!isPractice && (
          <div className="relative h-20 flex items-center">
            ...
            >
              <div className="flex flex-col items-center opacity-90">
                 ...
                 <div className={`relative ${isMoving(opponentProgress) ? 'engine-rumble' : ''}`}>
                    {isMoving(opponentProgress) && <div className="absolute -left-6 top-1/2 -translate-y-1/2 w-12 h-4 bg-rose-500/40 blur-md rounded-full nitro origin-right"></div>}
