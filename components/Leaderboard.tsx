...
const Leaderboard: React.FC<LeaderboardProps> = ({ entries, userFriends, onClose }) => {
  ...
  // Generate some realistic mock data for others
  const allEntries = useMemo(() => {
    const mockEntries: LeaderboardEntry[] = [];
    
    // Seed some mock data for each combination
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (activeTab === 'friends') {
      // Use the actual user friends list + the user themselves
      list = list.filter(e => e.isUser || userFriends.includes(e.name));
    }

    // Sort: Practice by accuracy, Endurance by Level then WPM, Race by WPM
